...
var _ = Describe("landep", func() {
	...
	It("works with cluster-pkg installer", func() {
		...
		targets := landep.Targets{"default": target}
		constraint, err := semver.NewConstraint(">= 1.0")
		Expect(err).To(Succeed())
		By("applies", func() {
			logs = nil
			parameter := landep.Parameter(nil)
			_, err = pkgManager.Apply(targets, "docker.io/pkgs/cluster", constraint, parameter)
			Expect(err).To(Succeed())
	...
	It("works with dependencies", func() {
		...
		By("applies", func() {
			logs = nil
			parameter := landep.Parameter(nil)
			_, err = pkgManager.Apply(targets, "docker.io/pkgs/cloud-foundry-environment", constraint, parameter)
			Expect(err).To(Succeed())
		...
		By("deletes", func() {
			...
			Expect(err).To(Succeed())
			Expect(logs).To(HaveLen(4))
			Expect(logs[0]).To(MatchRegexp("cf delete org \\w*"))
			Expect(logs[1]).To(MatchRegexp("kapp delete -n cf-system -a \\w*"))
			Expect(logs[2]).To(MatchRegexp("helm delete -n istio-system \\w*"))
			Expect(logs[3]).To(MatchRegexp("helm delete -n default \\w*"))
		})
	...
	It("deals with shared dependencies", func() {
		...
		By("applies cloud-foundry", func() {
			logs = nil
			parameter := landep.Parameter(nil)
			_, err = pkgManager.Apply(targets, "docker.io/pkgs/cloud-foundry", constraint, parameter)
			Expect(err).To(Succeed())
		...
		By("apply kyma", func() {
			...
			logs = nil
			parameter := landep.Parameter(nil)
			_, err = pkgManager.Apply(targets, "docker.io/pkgs/kyma", constraint, parameter)
			Expect(err).To(Succeed())
			Expect(logs).To(HaveLen(1))
		...
		By("deletes cloud-foundry and istio", func() {
			logs = nil
			err = pkgManager.Delete(targets, "docker.io/pkgs/cloud-foundry")
			Expect(err).To(Succeed())
			Expect(logs).To(HaveLen(1))
		...
		By("deletes kyma and istio", func() {
			...
			target := landep.NewK8sTarget("kyma-system", k8sConfig)
			targets := landep.Targets{"default": target}
			err = pkgManager.Delete(targets, "docker.io/pkgs/kyma")
			Expect(err).To(Succeed())
			Expect(logs).To(HaveLen(2))
